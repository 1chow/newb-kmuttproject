...
class bidForm extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		this.Validation(this.props.item,this.props.params,this.state.current,this.props.open,this.state.validates.endTime,this.state.validates.isActive)
	}

    Validation = (oldItem,itemId,current,open,endTime,life) => {
		var user = firebaseAuth().currentUser
		if (user) {
			let email = user.email
			let postBid = "http://us-central1-auctkmutt.cloudfunctions.net/bidOrder?itemId="+itemId+"&bid="+current+"&uId="+email
			if (life === 1){ //fininsh gate1 : Item expired
				if (oldItem._id === itemId){ //fininsh gate2 : Item is Equal
					if(oldItem.bid.current < current){ //fininsh gate3 : Check Over Old Cost?
